...
import { putUpdateUser,fetchComputer, fetchAccount } from '../../../services/AdminService';
...
const ModalTranfer = (props) => {
  ...
  const Tranfer = async () =>{
    let user =await fetchAccount(TranferID)
    if(user && user.userID){
        let res = await putUpdateUser(userID,AccountID,Name,Birth,ID,Phone,parseInt(dataUserEdit.money)-parseInt(Money));
        let tranfer = await putUpdateUser(TranferID,user.accountID,user.name,user.birth,user.id,user.phone,parseInt(user.money)+parseInt(Money));
        ...
        alert('Account id is not valid');
    }

    console.log("tranfer info:>> ",user)
  } 
    ...
    <>
      ...
      <Modal show={show} onHide={handleClose} animation={false}>
        <Modal.Header closeButton>
          <Modal.Title>Chuyển tiền</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        ...
        <div className="mb-3">
            <label className="form-label"> Tranfer To ACC ID</label>
            <input type="text" 
